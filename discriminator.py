...
def began_discriminator(D_I, batch_size, num_filters, hidden_size, image_size,
    ...
    with tf.variable_scope(scope_name) as scope:
        ...
        conv_0 = custom_conv2d(layer_1, 3, k_h=3, k_w=3, d_h=1, d_w=1)
        conv_0 = tf.nn.elu(conv_0)

        conv_1 =custom_conv2d(conv_0, num_filters, k_h=3, k_w=3, d_h=1, d_w=1)
        ...
        layer_2 = custom_conv2d(conv_2, 2 * num_filters, k_h=3, k_w=3, d_h=2, d_w=2)
        layer_2 = tf.nn.elu(layer_2)

        conv_3 = custom_conv2d(layer_2, 2 * num_filters, k_h=3, k_w=3, d_h=1, d_w=1)
        conv_3 = tf.nn.elu(conv_3)

        conv_4 =custom_conv2d(conv_3, 2 * num_filters, k_h=3, k_w=3, d_h=1, d_w=1)
        conv_4 = tf.nn.elu(conv_4)


        layer_3 = custom_conv2d(conv_2, 3 * num_filters, k_h=3, k_w=3, d_h=2, d_w=2)
        layer_3 = tf.nn.elu(layer_3)

        conv_5 = custom_conv2d(layer_3, 3 * num_filters, k_h=3, k_w=3, d_h=1, d_w=1)
        conv_5 = tf.nn.elu(conv_5)

        conv_6 =custom_conv2d(conv_5, 3 * num_filters, k_h=3, k_w=3, d_h=1, d_w=1)
        conv_6 = tf.nn.elu(conv_6)


        layer_4 = conv2d(conv_6, 4 * num_filters, k_h=3, k_w=3, d_h=2, d_w=2)
        layer_4 = tf.nn.elu(layer_4)

        conv_7 = custom_conv2d(layer_4, 4 * num_filters, k_h=3, k_w=3, d_h=1, d_w=1)
        conv_7 = tf.nn.elu(conv_7)

        conv_8 =custom_conv2d(conv_7, 4 * num_filters, k_h=3, k_w=3, d_h=1, d_w=1)
        conv_8 = tf.nn.elu(conv_8)

        if image_size == 64:
            enc = custom_fc(conv_8, hidden_size)
        else:
            layer_5 = conv2d(conv_8, 5 * num_filters, k_h=3, k_w=3, d_h=2, d_w=2)
            layer_5 = tf.nn.elu(layer_5)

            conv_9 = custom_conv2d(layer_5, 5 * num_filters, k_h=3, k_w=3, d_h=1, d_w=1)
            conv_9 = tf.nn.elu(conv_9)

            conv_10 =custom_conv2d(conv_9, 5 * num_filters, k_h=3, k_w=3, d_h=1, d_w=1)
            conv_10 = tf.nn.elu(conv_10)
            enc = custom_fc(conv_10, hidden_size)
